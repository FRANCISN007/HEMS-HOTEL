...
def list_issues_to_bar(
    ...
    try:
        ...
        response: List[store_schemas.IssueDisplay] = []

        # --------------------------------------------------
        # BUILD RESPONSE SAFELY
        # --------------------------------------------------
        for issue in issues:
            issue_items: List[store_schemas.IssueItemDisplay] = []
            ...
            for issue_item in issue.issue_items:
                ...
                # 🔑 Get selling price from bar inventory
                selling_price = (
                    db.query(bar_models.BarInventory.selling_price)
                    .filter_by(
                        bar_id=issue.bar_id,
                        item_id=item_obj.id
                    )
            ...
            bar_obj = db.query(store_models.Bar).filter_by(
                id=issue.bar_id
            ).first()
            ...
            response.append(
                store_schemas.IssueDisplay(
                    ...
                    issue_to="bar",
                    issued_to_id=issue.bar_id,
                    issued_to=store_schemas.BarDisplaySimple(
                        id=bar_obj.id,
                        name=bar_obj.name
                    ) if bar_obj else None,
                    issue_date=issue.issue_date,
