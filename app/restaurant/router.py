...
def create_meal_order(
    ...
    for it in order.items:
        ...
        if not inventory:
            # 🔥 create inventory record if missing (important for flexibility)
...
def update_meal_order(
    ...
    try:
        ...
        # --------------------------------------------------
        # 7️⃣ INSERT NEW ITEMS & DEDUCT STOCK
        # --------------------------------------------------
        for it in data.items:
            store_item = kitchen_map[it.store_item_id]
            qty_needed = it.quantity
            ...
            # 🔥 PRICE LOGIC
            price_per_unit = it.price_per_unit if it.price_per_unit and it.price_per_unit > 0 else store_item.selling_price
...
def summarize_items_sold(
    ...
    query = (
        ...
        .options(
            ...
            joinedload(RestaurantSale.payments)  # 🔥 IMPORTANT
        )
    )
    ...
    for sale in sales:
        ...
        for item in order.items:
            ...
            price = item.price_per_unit or store_item.selling_price or 0
            qty = item.quantity
        ...
        balance = total_amount - float(total_paid)

        payment_summary["total_sales"] += total_amount
