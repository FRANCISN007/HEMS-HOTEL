...
def update_bar_sale(
    ...
    try:
        # Track existing sale items
        existing_items = {si.bar_inventory.item_id: si for si in sale.sale_items}

        # Prepare requested items from payload
        requested_items = {}
        ...
        # Fetch inventories with lock, ONLY bar inventories
        inventories = {}
        for iid in all_item_ids:
            ...
            if not inv:
                raise HTTPException(status_code=404, detail=f"Item {iid} not found in this bar")
            inventories[iid] = inv

        # Update inventory and sale items
        for iid in all_item_ids:
            inv = inventories[iid]
            original_qty = existing_items.get(iid).quantity if existing_items.get(iid) else 0
            requested_qty = requested_items.get(iid)["quantity"] if requested_items.get(iid) else 0
            price = requested_items.get(iid)["selling_price"] if requested_items.get(iid) else 0.0

            # Restore inventory from original sale, then subtract new quantity
            inv.quantity = (inv.quantity + original_qty) - requested_qty

            # Remove old sale items
            if existing_items.get(iid):
                db.delete(existing_items[iid])

            # Add new sale items if quantity > 0
            if requested_qty > 0:
